...
7. **PASS — Processing failure retry.** `PipelineOrchestrator.ensure_seeded` now preserves a failed run with no result, and the API regression test creates a synthetic failure then retries that one email successfully. The drawer click remains covered by the global visual limitation.
